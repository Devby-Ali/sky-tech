    ...
    <div className="flex-center sm:justify-between flex-wrap flex-col mb-14 sm:mb-20 sm:flex-row gap-x-4 gap-y-7 text-darkColor dark:text-white">
      <div className="space-y-2 sm:space-y-3 sm:self-start mb-6 sm:mb-0">
        <div className="flex-center sm:justify-start gap-x-2.5">
          ...
          <span className="hidden sm:block w-1 h-16 bg-sky-600 rounded-r-full shadowLightBlue ml-2 mt-3"></span>
          <h3 className="font-EstedadBold text-[2.4rem] sm:text-[2.7rem]">
            {title}
          </h3>
        </div>
        {desc ? (
      ...
      {btnTitle ? (
        <div className="courses-header__left sm:self-end sm:ml-8">
          <Link
            to={`/${btnHref}`}
            className="relative flex text-[1.7rem] md:text-3xl tracking-tighter text-sky-700 hover:text-purple-200 group"
          >
            <div className="font-EstedadMedium pb-2 pl-5 border-b-2 border-b-sky-700 group-hover:border-b-pink-200 transition-colors">
              {btnTitle}
            </div>
